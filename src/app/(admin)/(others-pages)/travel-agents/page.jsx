...
const page = () => {
  const travelPageTrans = useTranslations("travelagentpage");
  ...
  const debouncedFetch = useMemo(() => {
    return debounce((filter) => {
      getTravelAgentsData(filter, 0, 40);
    }, 600);
  }, []);
    ...
    <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] lg:p-6">
      <div className="flex flex-col items-center justify-between gap-4 mb-4 md:flex-row">
        <h3 className="text-lg font-semibold text-gray-800 lg:text-2xl dark:text-white/90">
          {travelPageTrans("Travel Agents")}
        </h3>
      </div>

      <div className="flex flex-row items-center justify-between gap-[10px] flex-wrap lg:flex-nowrap pb-[20px]">
        <StatCard
          title={travelPageTrans("TotalUsers")}
          value={travelAgentsCardsData?.stats?.totalUsers || 0}
          icon={<UserIcon />}
          color="bg-indigo-100 text-indigo-600"
        />
        <StatCard
          title={travelPageTrans("TotalAgents")}
          value={travelAgentsCardsData?.stats?.totalTravelAgents || 0}
          icon={<UserIcon />}
          color="bg-indigo-100 text-indigo-600"
        />
        <StatCard
          title={travelPageTrans("BlockedUsers")}
          value={travelAgentsCardsData?.stats?.totalBlockedUsers || 0}
          icon={<Block />}
          color="bg-red-100 text-red-600"
        />
        <StatCard
          title={travelPageTrans("ActiveAgents")}
          value={
            travelAgentsCardsData?.stats?.totalTravelAgents -
              travelAgentsCardsData?.stats?.totalBlockedUsers || 0
          }
          icon={<UnBlock />}
                  ...
                  <Checkbox
                    onChange={(cond) => {
                      if (cond) {
                        const ids = [];
                        const emails = [];
                        travelAgentsCardsData.data.forEach((it) => {
                          ids.push(it._id);
                    ...
                    checked={travelAgentsCardsData.data.every((it) =>
                      selectedIds.includes(it._id)
                    )}
                  />
                  {travelPageTrans("User")}
                </th>
                <th className="med-16 text-start pl-[6px] py-[15px] !text-gray-700 dark:!text-gray-300">
                  {travelPageTrans("UserRole")}
                </th>
                <th className="med-16 text-start pl-[6px] py-[15px] !text-gray-700 dark:!text-gray-300">
                  {travelPageTrans("Email")}
                </th>
                <th className="med-16 text-start pl-[6px] py-[15px] !text-gray-700 dark:!text-gray-300">
                  {travelPageTrans("Phone")}
                </th>
                <th className="med-16 text-start pl-[6px] py-[15px] !text-gray-700 dark:!text-gray-300">
                  {travelPageTrans("CreatedAt")}
                </th>
                <th className="med-16 text-start pl-[6px] py-[15px] !text-gray-700 dark:!text-gray-300">
                  {travelPageTrans("Status")}
                </th>
                <th className="med-16 text-start pl-[6px] py-[15px] !text-gray-700 dark:!text-gray-300">
                  {travelPageTrans("Actions")}
                </th>
              </tr>
            </thead>
            <tbody>
              {travelAgentsCardsData.data?.map((user, index) => (
                <UserCard
                  key={index}
                  user={user}
                  selectedIds={selectedIds}
                  setSelectedIds={setSelectedIds}
                  travelAgentsCardsData={travelAgentsCardsData}
                  setSelectedEmails={setSelectedEmails}
                  getMoreData={getTravelAgentsData}
                  last={index === travelAgentsCardsData.data.length - 1}
                  loading={loading}
                  showType={false}
                  NotificationClick={NotificationClick}
                  ...
                  clickFuncGrantAd={clickFuncGrantAd}
                  userCardsData={travelAgentsCardsData}
                />
              ))}
            </tbody>
          </table>
          {travelAgentsCardsData.data.length < 1 && (
